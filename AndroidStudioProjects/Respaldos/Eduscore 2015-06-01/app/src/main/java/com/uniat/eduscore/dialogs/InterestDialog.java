...
public class InterestDialog extends DismissDialog {
    ...
    private void generateListInterest(){
        for( int i = 0; i < typesCareers.length; i++ ){
            ...
            for(int j = 0; j < intereses.length; j++){
                ...
                ItemInterest interes = new ItemInterest(intereses[j], imgBtnCheck, textInterest, false);
    ...
    public void onClick(View v) {
        ...
        for( int i = arrayInterests.size() - 1; i > 0; i-- ){
            if( arrayInterests.get(i).isStatus() ){
                ...
                MainActivity.mthis.listIntereses.add(arrayInterests.get(i).getName());
            }
        }
    ...
    private String[] getInterests(String type){
        ...
        careerDAO.open();
        String sql = "SELECT * FROM " + EduscoreOpenHelper.TABLE_ESC_CAREER + " WHERE idCareerType = '" + type + "' AND prefix = '1'";//prefix == status
