...
public class CotizacionTable extends Activity{
	...
	private double[] calcularProspeccion( double in, double ca, double exc, int n_per ){
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Prospeccion = calcularProspeccion(.06, .005, myEstado.getPrimaExcedente(), 20);
		cotizacion();
		headerBar();
	}
	...
	public void calculo( View pView ){
		pView.setOnClickListener(new OnClickListener() {			
			@Override
			public void onClick(View v) {
				in = ( inInput.getText().toString().equals("") ? 6 : Double.parseDouble( inInput.getText().toString() )) / 100 ;
				ca = ( caInput.getText().toString().equals("") ? .5 : Double.parseDouble( caInput.getText().toString() )) / 100 ;
				Prospeccion = calcularProspeccion( in, ca, myEstado.getPrimaExcedente(), 20 );
				cotizacion();
			}
		});
	}
