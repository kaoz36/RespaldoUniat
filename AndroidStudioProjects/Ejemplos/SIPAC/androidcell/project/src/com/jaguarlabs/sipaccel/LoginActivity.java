...
import com.jaguarlabs.sipaccel.util.DelayedExtenededInitter;
...
public class LoginActivity extends ExtendedFragmentActivity implements IJSONResponseHandler, DelayedExtenededInitter{
	...
	public static class ExtendedTextWatcher implements TextWatcher{
		...
		public ExtendedTextWatcher( ImageButton pButton ){
			showButton = pButton;
		}
	...
	public static class ExtendedClickListener implements OnClickListener{
	...
	protected void init(){
		...
		ImageButton salirButton = (ImageButton)findViewById(R.id.btn_salir);
	...
	public void onResponse(JSONObject jsonResponse)throws Exception {
		...
		}else
		{
			if(jsonResponse.has("usuario") && jsonResponse.has("promotoria") && 
				...
				//Si el Usuario cambio de promotoria
				String idPromo = shared.getString("id_promot", "").equals("") ? "1" : shared.getString("id_promot", "") ;
				if(!DataModel.getInstance().getPromotoria().getIdPromotoria().equals( idPromo ) )
				{
					dialog = ProgressDialog.show( this , "Progreso", "Borrando Datos", true);
	...
	private void guardarPreferencias(){
		...
		datos.putLong("fecha", new Date().getTime() );
