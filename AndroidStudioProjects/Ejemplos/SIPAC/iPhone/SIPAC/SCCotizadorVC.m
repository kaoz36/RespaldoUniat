...
- (void)viewDidLoad
{
    ...
    self.valor20Salarios = [[SCDatabaseManager sharedInstance] getMinimunSalary] * 20 * 30;
    self.valor200Salarios = self.valor20Salarios * 10;
    
    self.salarioMinimo.userInteractionEnabled = NO;
    self.salarioMinimo.textColor = [UIColor blackColor];
    self.salarioMinimo.text = [NSString stringWithFormat:@"%f", [[SCDatabaseManager sharedInstance] getMinimunSalary]];
...
-(void)selectPago {
    ...
    if (self.cotizador.esPrima) {
        ...
        if (self.comboFormaDePago.index == 0) {
            Prima = Prima * 1;
        } else if (self.comboFormaDePago.index == 1) {
            Prima = Prima * 12;
        } else if (self.comboFormaDePago.index == 2) {
            Prima = Prima * 24;
        }
...
-(void)textFieldDidEndEditing:(UITextField *)textField {
    ...
    if (textField.tag == 2) {
        ...
        if (sumaAseguradaBas > sumaAseguradaTextField) {
            [self recalculoConBotonSeleccionado:YES];
            if (self.btnSelectBacy.selected == YES) {
              [self initCotizador];
            }
        }
...
-(IBAction)BACYSelected:(id)sender {
    [self metodoGenericoParaBotonConBacy:sender];
    if (!self.cotizador.esPrima) {
...
- (void)metodoGenericoParaBotonConBacy:(id)sender
{
    UIButton *button = sender;
    [button setSelected:!button.isSelected];
    [self recalculoConBotonSeleccionadoConBacy:button.isSelected];
...
- (void)recalculoConBotonSeleccionadoConBacy:(BOOL)seleccionado{
...
-(void)initCotizadorSinBacy{
        ...
        coberturaGFA.min = !self.esNuevaPoliza ? 0 : self.valor20Salarios;
        ...
        coberturaBACY.maxMenores = self.valor200Salarios;
        coberturaBACY.maxMayores = self.valor200Salarios;
        ...
        coberturaBACY.valorLimiteConsiderandoBAS = self.valor200Salarios;
        coberturaBACY.considerarBAS = considerarBAS;
        coberturaBACY.limiteSumaAseguradaParaCotizadorPorPrima = self.valor200Salarios;
        ...
        coberturaGFC.min = !self.esNuevaPoliza ? 0 : self.valor20Salarios;
        ...
        coberturaGFH.min = !self.esNuevaPoliza ? 0 : self.valor20Salarios;
    ...
    coberturaGFH.indexSeleccionGFH = self.comboHijos.index;
        ...
        coberturaCP1.sexIndex = self.comboCancerPlus1.index;
        ...
        coberturaCP2.sexIndex = self.comboCancerPlus2.index;
        ...
        coberturaCP3.sexIndex = self.comboCancerPlus3.index;
        ...
        if (self.comboFormaDeCalculo.index == 1) {
            isPrima = YES;
        } else {
            isPrima = NO;
        }
    self.cotizador = [[SCCotizador alloc]initWithCoberturas:coberturas prima:isPrima];
    self.cotizador.valor20Salarios = self.valor20Salarios;
    self.cotizador.valor200Salarios = self.valor200Salarios;
...
- (void)cotizador:(SCCotizador *)aCotizador didGetExtraPrimaText:(NSString *)nuevaExtraPrima {
    if([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad) {
        ...
        self.extraPrimaIphone.text = nuevaExtraPrima;
        [self.extraPrimaIphone setTextColor:[UIColor redColor]];
        [self.extraPrimaIphone setFont:[UIFont systemFontOfSize:14.0]];
    }
}
...
-(NSDictionary *)datosProyeccionFinanciera{
    ...
    int multiplyer = 1;
    if (!self.cotizador.esPrima) {
        if (self.comboFormaDePago.index == 1) {
            multiplyer = 12;
        }
        if (self.comboFormaDePago.index == 2) {
            multiplyer = 24;
        }
    }else{
        if (self.comboFormaDePago.index == 1) {
            primaTotal = primaTotal / 12;
        }
        if (self.comboFormaDePago.index == 2) {
            primaTotal = primaTotal / 24;
        }
    }
    NSString *primaTotalAnual = [NSString stringWithFormat:@"%.2f", primaTotal];
    
    NSString *primaDiaria = [NSString stringWithFormat:@"%.2f", ([self.primaTotalAnual.text doubleValue] * multiplyer)/360];
