...
- (NSMutableArray *) getRowsFromColumnName: (NSString *)ColumnName andValue:(NSString*)Value {
    NSString * query = [NSString stringWithFormat:@"select * from %@ where %@ = '%@'", tableName,ColumnName,Value];
...
- (BOOL)checkIfColumnExists:(NSString*)column InTable:(NSString*)Table{
    ...
    sqlite3_stmt *statementChk;
    const char *query = [[NSString stringWithFormat:@"SELECT %@ FROM %@",column,Table] UTF8String];
    sqlite3_prepare_v2(database, query, -1, &statementChk, nil);
...
- (void)addColumn:(NSString*)columnName andDefaultValue:(id)defaultValue inTable:(NSString*)table{
    if ([defaultValue isKindOfClass:[NSString class]]) {
        [self doQuery:[NSString stringWithFormat:@"ALTER TABLE %@ ADD COLUMN %@ DEFAULT '%@'",table,columnName,defaultValue]];
        return;
    }
...
- (NSNumber *) lastInsertId {
    return [NSNumber numberWithInt: (int)sqlite3_last_insert_rowid(database)];
}

#pragma mark -
#pragma mark Transactions Method

- (void)doTransactionsWithParams:(NSString*)params andValues:(NSArray*)values{
    
    sqlite3_exec(database, "BEGIN EXCLUSIVE TRANSACTION", 0, 0, 0);
    
    for (int i = 1; i < values.count; i++)
    {
        NSString *query = [NSString stringWithFormat:@"insert into %@ (%@) values (%@)",tableName,params,values[i]];
        [self doQuery:query];
    }
