...
- (void)viewDidLoad
{
    ...
    if(!([UIDevice currentDevice].userInterfaceIdiom==UIUserInterfaceIdiomPad)) {
        self.scrollIphone.contentSize = CGSizeMake(320, 940);
    ...
    [self.btnZona setTitle:@"Escoge una opcion.." forState:UIControlStateNormal];
...
//    //Zona
//    
//    if ([self.btnZona.titleLabel.text isEqualToString:@"Escoge una opcion.."]) {
//        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Aviso" message:@"Por favor selecciona una zona." delegate:Nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
//        [alert show];
//        return;
//    }else if ([self.btnZona.titleLabel.text isEqualToString:@"Todas"]){
//        fields = [fields stringByAppendingString:@"zona, "];
//    }else{
//        fields = [fields stringByAppendingString:@"zona, "];
//        values = [values stringByAppendingString:[NSString stringWithFormat:@"and zona like('%%%@%%') ",self.btnZona.titleLabel.text]];
//    }
...
//    }
    ...
    if (prevalue) {
        query = [NSString stringWithFormat:@"SELECT * FROM %@ WHERE %@ ",AllProspectTable,values];
    }else{
...
- (void)loadWebView{
    ...
    CGRect frame = CGRectMake(self.view.frame.origin.x, self.view.frame.origin.y + 5, self.view.frame.size.width, self.view.frame.size.height - 5);
    resultsProspect = [[UIWebView alloc] initWithFrame:frame];
    NSLog(@"resulsProspect.frame : %@",NSStringFromCGRect(resultsProspect.frame));
