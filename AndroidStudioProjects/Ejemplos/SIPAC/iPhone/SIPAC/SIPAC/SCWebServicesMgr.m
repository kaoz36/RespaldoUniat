...
- (void)SWSaveConsultLocationWithDelegate:(id)theDelegate{
...
- (void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation{
    ...
    NSDictionary *profile = [[SCDatabaseManager sharedInstance] GetProfile];
    //[29/05/13 09:17:31] Raul Acevedo:  YYYY-MM-DD
...
+ (void)parseLogin:(NSDictionary*)dictionary userName:(NSString*)userName userPass:(NSString*)userPass{
    ...
    NSDictionary *Dic = @{@"id_user":dictionary[@"usuario"][@"Agente"],@"email":dictionary[@"usuario"][@"correo"], @"full_name_user":dictionary[@"usuario"][@"Nombre"], @"rfc":dictionary[@"usuario"][@"RFC"], @"fecha_nacimiento":@"", @"fecha_expiracion_user":@"", @"monedero_user":@"" , @"logged_user":@"1",@"promotoria":dictionary[@"promotoria"][@"promot"], @"expiracion":dictionary[@"configuracion"][@"expiracion"], @"id_configuracion":dictionary[@"configuracion"][@"id_configuracion"], @"salario":dictionary[@"configuracion"][@"salario"], @"loginDate":[NSString stringWithFormat:@"%f",[[NSDate date] timeIntervalSince1970]],@"user_name":userName, @"user_pass":userPass};
...
- (void)dispachQueue{
    if (!onRequest) {
        ...
        downloadQueue[0] = [downloadQueue lastObject];
...
- (void)getAllValuesWithPaginatedCompletition:(int)paginate withURL:(NSString*)url inWS:(NSString*)webS Tag:(int)tag Current:(int)current Total:(int)total withWSCompletition:(BOOL)YESNO{
    ...
    BWConnectionManager *connection = [[BWConnectionManager alloc] initWithJSONPost:@{@"":@""} withUrl:url inWS:wsCompletition delegate:self message:[NSString stringWithFormat:@"Descargando datos... \n (%d de %d)",current,total] withHud:YES requestWithTag:tag ShowResponse:NO Resting3g:YES];
...
-(void)didGetResponse:(id)response inConnectionTag:(int)tag{
    ...
    switch (tag) {
        ...
        case kConnectionTagGetAllCoberturas:
            ...
            for (int x = 1; x < dataArray.count; x++) {
                [[SCDatabaseManager sharedInstance] doPaginateInsertionWithFields:dataArray[0] values:dataArray[x] forTable:CoberturaTableName];
            }
        ...
        case kConnectionTagGetAllServicios:
            ...
            NSLog(@"enter for");
            for (int x = 1; x < dataArray.count; x++) {
                [[SCDatabaseManager sharedInstance] doPaginateInsertionWithFields:dataArray[0] values:dataArray[x] forTable:ServicioGeneralTableName];
            }
            NSLog(@"out of for");
        ...
        case kConnectionTagGetAllServiciosVenta:
            ...
            for (int x = 1; x < dataArray.count; x++) {
                [[SCDatabaseManager sharedInstance] doPaginateInsertionWithFields:dataArray[0] values:dataArray[x] forTable:ServicioVentaTableName];
            }
        ...
        case kConnectionTagGetAllServiciosInterno:
            ...
            for (int x = 1; x < dataArray.count; x++) {
                [[SCDatabaseManager sharedInstance] doPaginateInsertionWithFields:dataArray[0] values:dataArray[x] forTable:VentaInternoTableName];
            }
        ...
        case kConnectionTagGetAllProspection:
            ...
            for (int x = 1; x < dataArray.count; x++) {
                [[SCDatabaseManager sharedInstance] doPaginateInsertionWithFields:dataArray[0] values:dataArray[x] forTable:AllProspectTable];
            }
