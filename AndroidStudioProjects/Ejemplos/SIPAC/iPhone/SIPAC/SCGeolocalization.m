...
- (IBAction)accept:(id)sender {
    ...
    if (self.txtName.text.length == 0 || self.txtProspectos.text.length == 0 || self.txtRetenedor.text.length == 0 || self.txtUpago.text.length == 0 || self.txtZona.text.length == 0 || self.txtDireccion.text.length == 0) {
    ...
    SCCustomPin *annot = [[SCCustomPin alloc] initWithName:[NSString stringWithFormat:@"%@ (%@ prospectos)",self.txtName.text,self.txtProspectos.text] address:[NSString stringWithFormat:@"Retenedor : %@, U. de pago : %@",self.txtRetenedor.text,self.txtUpago.text] placeId:@"" coordinate:touchMapCoordinate];
