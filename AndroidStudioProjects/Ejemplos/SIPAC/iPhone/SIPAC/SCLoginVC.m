...
- (void)viewDidLoad
{
    ...
    self.txtName.text = profile[@"user_name"];
}
...
- (IBAction)login:(id)sender {
    if ([BWConnectionManager connectedToNetworkRestring3g:NO]) {
        ...
        if ([[profile[@"user_name"] lowercaseString] isEqualToString:[self.txtName.text lowercaseString]] && [profile[@"user_pass"] isEqualToString:self.txtPass.text]) {
...
-(void)didGetResponse:(id)response inConnectionTag:(int)tag{
    ...
    if (error != nil) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Error" message:error delegate:nil cancelButtonTitle:@"Aceptar" otherButtonTitles: nil];
        [alert show];
        if([response[@"erase"] integerValue] == 1){
            [[SCDatabaseManager sharedInstance] deleteTables];
        }
        return;
    }
