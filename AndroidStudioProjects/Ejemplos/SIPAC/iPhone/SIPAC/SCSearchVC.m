...
- (void)viewDidLoad
{
    ...
    originalFrame = CGRectMake(self.searchBar.frame.origin.x+10, self.searchBar.frame.origin.y+7, self.searchBar.frame.size.width-20, 0);
...
- (void)searchOnAutoSugest:(BOOL)autosugest{
    ...
    if (![searchText isEqualToString:@""] && searchText.length != 1) {
        ...
        NSString *consulta = [NSString stringWithFormat:@"SELECT id_poliza, nombre_poliza , rfc_poliza , emp_poliza, prima_poliza , ret_poliza , ult_pago_poliza FROM Poliza WHERE (nombre_poliza like('%%%@%%') %@%@%@) Order by nombre_poliza COLLATE NOCASE ASC limit 0,%d",search,campo_rfc,campo_no_empl,campo_poliza,limit];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (cell == nil) {
        ...
        UITextView *textView = [[UITextView alloc] initWithFrame:CGRectMake(0, 0, self.searchBar.frame.size.width-10, 40)];
    ...
    UITextView *tmpTextView = (UITextView*)[cell.contentView viewWithTag:1];
    tmpTextView.text = [NSString stringWithFormat:@"%@ - %@",[dict objectForKey:@"id_poliza"],[dict objectForKey:@"nombre_poliza"]];
